-- Preserve one question per spelling while accepting meanings from duplicate rows.
begin;
...
begin
  ...
  loop
    select q.id, w.english, w.korean,
      coalesce(w.accepted_answers,'{}'::text[]) || coalesce(q.accepted_answers,'{}'::text[]) as accepted_answers,
...
as $$ select jsonb_build_object('mixed_questions',true,'duplicate_meanings',true) $$;
