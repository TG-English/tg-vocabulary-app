...
$('#nextQuestionBtn').addEventListener('click',()=>{const answer=$('#answerInput').value.trim();if(!answer)return toast('정답을 입력해주세요.');const word=state.questions[state.index],correct=state.current.type==='en-ko'?word.korean:word.english;const accepted=state.current.type==='en-ko'?correct.split(/[,;/·]| 또는 /).map(v=>v.trim()).filter(Boolean):[correct];const isCorrect=state.current.type==='en-ko'?accepted.some(v=>koreanAnswerMatches(answer,v)):accepted.some(v=>normalize(v)===normalize(answer));state.answers.push({word,answer,correct,isCorrect});if(++state.index<state.questions.length)renderQuestion();else finishTest()});
